...
```{r}
lm1 <- lm(grocery_100k ~ Pct_white, data= chicago)
plot(lm1, 1)
summary(lm1)
```
...
```{r}
ggplot(data = chicago, aes(x = Pov14, y = TeenBirth)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

lm2 <- lm(Pov14 ~ TeenBirth, data = chicago)
plot(lm2, 1)
summary(lm2)
```

```{r}
chicago$resid2 <- residuals(lm2)

ggplot(chicago) +
  geom_sf(aes(fill = resid2), color = "white") + 
  scale_fill_gradient2()

moran(chicago$resid2, 
                       chicago_nbw, 
                       n = length(chicago_nb), 
                       S0 = Szero(chicago_nbw))

sarlm2 <- lagsarlm(Pov14 ~ TeenBirth, data = chicago, listw = chicago_nbw)

summary(sarlm2)
```

```{r}
lm3 <- lm(log(PropCrRt) ~ PerCInc14 + Unemp14 + HSGrad14, data = chicago)
plot(lm3, 1)
summary(lm3)

sarlm3 <- lagsarlm(log(PropCrRt) ~ PerCInc14 + Unemp14 + HSGrad14, data = chicago, listw = chicago_nbw)
summary(sarlm3)
```
